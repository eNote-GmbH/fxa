...
export class CurrencyManager {
  ...
  constructor(config: ConfigType) {
    ...
    for (const [currency, countries] of currencyMap) {
      ...
      if (!CurrencyManager.validCurrencyCodes.includes(currency)) {
        throw new Error(`Currency code ${currency} is invalid.`);
      }
      if (
        this.payPalEnabled &&
        !CurrencyManager.supportedPayPalCurrencies.includes(currency)
      ) {
        throw new Error(`Currency code ${currency} is invalid.`);
      }
      // Are countries acceptable
      for (const country of countries) {
        if (!CurrencyManager.validCountryCodes.includes(country)) {
