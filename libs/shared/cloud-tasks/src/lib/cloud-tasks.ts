...
export class CloudTasks {
  ...
  public get queueEnabled() {
    ...
    // Previously, the following rule was in place, but now that the cloud task
    // emulator has been setup, the cloud task queue can be used locally as well, so
    // this is no longer needed.
    //  ['stage', 'prod'].includes(this.config.env);
