...
export class SigninReactPage extends BaseLayout {
  ...
  async fillOutAuthenticationForm(code: string): Promise<void> {
    await expect(
      this.authenticationFormHeading,
      'The Authentication form heading is missing or not as expected. ' +
  ...
  async fillOutPasswordForm(password: string): Promise<void> {
    await expect(
      this.passwordFormHeading,
      'The Password form heading is missing or not as expected. ' +
        'Are you filling out the right form?'
    ).toBeVisible();
