...
export class CapabilityService {
  ...
  private async configuredSubscribedMergedConfigs(subscribedPrices: string[]) {
    const paymentConfigManager = this.paymentConfigManager;
    if (!paymentConfigManager) return [];
    const allPlans = (await paymentConfigManager.allPlans()).filter((plan) =>
      subscribedPrices.includes(plan.stripePriceId ?? '')
    );
    return allPlans.map((plan) => {
      return paymentConfigManager.getMergedConfig(plan);
    });
  }
