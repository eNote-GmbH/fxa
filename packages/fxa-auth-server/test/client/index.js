...
module.exports = (config) => {
  ...
  Client.prototype.setupCredentialsV2 = async function (email, password, originalLoginEmail) {
    ...
    // Note that using originalLoginEmail is to main parity with V1. In reality, V2 no longer
    // requires the notion of an original email. Rather the correct email should just be
    ...
    // If the client doesn't have a known salt, we will construct one here.
    if (!clientSalt) {
    ...
    // Now that we have passed through all derivations successfully apply state.
  ...
  Client.prototype.auth = function (opts) {
    if (!this.authPW) {
      throw new Error('authPW not set. Aborting operation!')
    }
  ...
  Client.prototype.upgradeCredentials = async function (newPassword, headers) {
    ...
    // TODO: Need to update kB, it should be the same...
    if (!this.kB) {
      await this.getKeysV1();
    }

    // Create new v2 credentials with current password, and finish reset
    await this.setupCredentialsV2(this.email, newPassword);
    ...
    const resFinish = await this.api.passwordChangeFinishV2(
      ...
      this.clientSalt,
      headers || {},
      // Note that a session token is needed to get keyFetch and keyFetchToken2 back.
      undefined
    );
  ...
  Client.prototype.generateNewWrapKb = function () {
    ...
    // account creation, or any other time a new wrapKb needs
    // to be generated.
    //
