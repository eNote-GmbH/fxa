...
describe('/linked_account', function () {
  ...
  describe('/linked_account/webhook/google_event_receiver', () => {
    ...
    it('handles tokens revoked event', async () => {
      ...
      assert.calledWithExactly(
        statsd.increment,
        'handleGoogleSET.processed.https://schemas.openid.net/secevent/risc/event-type/tokens-revoked'
      );
      assert.calledWithExactly(
        mockLog.debug,
        'Revoked 1 third party sessions for user fxauid'
    ...
    it('handles token revoked event', async () => {
      ...
      assert.calledWithExactly(
        statsd.increment,
        'handleGoogleSET.processed.https://schemas.openid.net/secevent/risc/event-type/token-revoked'
      );
      assert.calledWithExactly(
        mockLog.debug,
        'Revoked 1 third party sessions for user fxauid'
