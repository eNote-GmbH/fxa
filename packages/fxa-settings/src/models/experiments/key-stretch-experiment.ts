...
export class KeyStretchExperiment extends ModelDataProvider {
  ...
  isV2(config: Pick<Config, 'featureFlags'>) {
    // If stretch=2 in URL, then force V2 key stretching for this page render,
    // regardless of all other settings / configs. This is used mostly for
    // dev/test purposes.
    if (this.stretch === '2') {
      return true;
    }
 ...
 * porting over experimentation code into settings, but this in flux at
 * the moment. See FXA-XXXX for more info and latest status
 */
