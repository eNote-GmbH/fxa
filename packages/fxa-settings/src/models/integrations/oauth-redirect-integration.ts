...
/**
 * Handles logic around generating a redirect for relying part after a sucessfull password reset.
 */
export class OAuthRedirectIntegration {
  ...
  /**
   * After a passoword reset, this code can be used to generate a redirect link which relays the new oauth token to the relying party.
  ...
  public async handlePasswordReset(
    ...
    const { kB } = await this.authClient.accountKeys(keyFetchToken, unwrapKB);
  ...
  private async constructKeysJwe(
    ...
    if (
      ...
      const clientKeyData = await this.authClient.getOAuthScopedKeyData(
        sessionToken,
        this.relier.clientId,
        this.relier.scope
      );
  ...
  private async constructOAuthCode(sessionToken: string, keysJwe: any) {
    ...
    const result = await this.authClient.createOAuthCode(
      sessionToken,
      this.relier.clientId,
      this.relier.state,
      opts
