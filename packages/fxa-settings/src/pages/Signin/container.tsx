  ...
  const beginSigninHandler: BeginSigninHandler = useCallback(
    async (email: string, password: string) => {
      ...
      if (keyStretchExp.queryParamModel.isV2()) {
        ...
        if (credentialStatusData.data?.credentialStatus.upgradeNeeded) {
          ...
          if (keyFetchToken) {
            try {
              ...
              if (uiError.error.errno === 104) {
                // If the account is simply 'unverified', then go through normal flow.
              } else {
                // Something went wrong, don't fail, so user can still log in, but report it to sentry;
                Sentry.captureMessage(
                  'Failure to finish v2 upgrade, because getWrappedKeys failed.'
                );
          ...
          if (wrapKb && passwordChangeToken) {
            try {
              ...
              // Something went wrong, don't fail, so user can still log in, but report it to sentry;
              Sentry.captureMessage(
                'Failure to finish v2 upgrade, because passChangeFinish failed.'
              );
              Sentry.captureException(error);
            }
          }
        ...
        if (v2Credentials) {
          console.log('!!! Begin sign in with V2 credentials');
          try {
          ...
          console.log('!!! Failed to create V2 credentials');
          Sentry.captureMessage(
