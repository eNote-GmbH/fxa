...
const Signup = ({
  ...
  useEffect(() => {
    if (webChannelEngines) {
      if (isSyncDesktopIntegration(integration)) {
        // Desktop web channel message offers additional engines
        ...
        // Mobile web channel message offers all engines
        setOfferedSyncEngineConfigs(
  ...
  useEffect(() => {
    if (offeredSyncEngineConfigs) {
      const defaultSelectedSyncEngines = offeredSyncEngineConfigs
        .filter((engineConfig) => engineConfig.defaultChecked)
        .map((engineConfig) => engineConfig.id);
      setDeclinedSyncEngines(defaultSelectedSyncEngines);
    }
      ...
      if (data) {
        ...
        if (isSyncDesktopIntegration(integration)) {
          const offeredSyncEngines =
            offeredSyncEngineConfigs?.map((engineConfig) => engineConfig.id) ||
            [];
          firefox.fxaLogin({
            email: queryParamModel.email,
  ...
  console.log('offeredSyncEngineConfigs', offeredSyncEngineConfigs);
